...
一次供应商故障弄丢了 2026-08-12 的条目。加个备用源是显而易见的修法,而**不验证就上线,就是让上一版
spec 退役的那个错误再犯一次**。所以同一条冻结规则,在两个不同供应商的后复权A股面板上
...
那个空洞保持原样,由解释而不是由填补来处理,[数据已公开](docs/data/source_reconciliation.json)。
