...
| 区间 | 预测均值 | 实现均值 | OOS R² vs 训练均值 | 日 Rank IC | IC HAC t |
|---|---:|---:|---:|---:|---:|
| Train walk-forward | 0.2733% | 0.7768% | -0.0033 | 0.0215 | 1.8465 |
| Validation | -0.4113% | 1.3243% | -0.1212 | 0.0827 | 2.3281 |
| Shadow | 1.8648% | -1.5874% | -0.0861 | -0.0305 | -0.8855 |

Validation 中模型能做一定截面排序，但绝对收益水平预测错误；到了 shadow，预测均值为正而实现均值为负，截面 Rank IC 也转负。这说明“预计涨幅”不能当作可信目标价或保证收益。

主策略和完全相同日期的 V2 Ridge Top10 对比：

| 区间 | 信号日 | V4均值 | V4胜率 | V4尾亏率 | 同日均值增量 | 同日胜率增量 | 收益差 HAC t | 扣成本累计 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Validation | 68 | 1.5975% | 75.00% | 4.41% | +0.1411% | +1.47pp | 0.7573 | +7.71% |
| Shadow | 102 | -0.9845% | 40.20% | 40.20% | -0.1889% | +1.96pp | -0.8885 | -11.68% |

V4 在 shadow 略微提高胜率和降低尾亏率，但平均收益与扣成本收益变差，说明它偏向“多赢少赚、少数亏损仍然较大”，没有达到整体收益改进。

2026-08-03 的 Top10 平均原始预测收益为 5.41%，因而形式上通过 0.30% 闸门；但该预测远高于当前滚动训练先验 0.76%，且 shadow 已证明模型会在下跌环境中明显过度预测。因此当日闸门结果仅保留为诊断，不能成为买入授权。

最终判定未通过的核心原因：validation 与 shadow 的 OOS R² 都为负；shadow Rank IC 为负；shadow 平均收益和扣成本累计收益为负；同日收益增量没有通过 HAC 显著性。V4 不接入交易、不改变当前生产模型，也不依据已查看窗口重新优化三头权重。
