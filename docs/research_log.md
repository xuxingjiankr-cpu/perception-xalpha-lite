...
| 17 | `qlib158/vstd60` is mislabelled and really ranks volume LEVEL, not volume stability | `research_vstd60_specification_audit_v1.py`, daily cross-sectional Spearman vs 4 comparators, both windows | ✅ not falsified | **My prior was wrong and the data said so.** The vendor formula is `ts_std(volume,60)/volume` — divided by TODAY's volume, not the 60-day mean — and `vendor/true_cv ≡ mean(V,60)/V_t` exactly, so a quiet session scores strictly higher on identical trailing dispersion. I expected that to dominate. It does not: vstd60 correlates **+0.495 (validation) / +0.558 (shadow)** with a true coefficient of variation against only **+0.268 / +0.217** with inverse current volume. The rolling-std numerator dominates; the label is accurate. **The useful finding is the orthogonality**: vstd60 vs 20-day realised price volatility is **−0.037 / −0.074** — essentially uncorrelated. So #16's side diagnostic (vstd60 5.27pp vs RV20 12.76pp on tail ranking) was never a specification bug: volume stability and tail risk are close to unrelated quantities, and the book spends **9.6% of its weight** on a correctly-defined factor that is orthogonal to what #13/#14 showed actually ranks tail risk. Correlation only; establishes no edge, and #12 caps what any single-factor result can mean |
