...
[工具首页](README_CN.md) · [数据来源边界](DATA_PROVENANCE.md)

以下内容从原首页移出，用于区分实测记录与合成教学案例。此次保留不是重新验证，也不是交易认证。
历史研究及抽样前瞻记录不是合成数据；原有局限继续适用。本次未增加私人研究结果。
标记区块仍由原记录任务更新。

## 预测在它对应的那个交易日之前公布

[![轮动记录](daily-rotation.svg)](https://xuxingjiankr-cpu.github.io/perception-xalpha-lite/#live)

一个冻结的规则(`dea0e608`)从约 4700 只合格股票里选出一只,带时间戳提交到这里,
**在那个市场开盘之前**。自 2026-08-22 起,runner 改为**每周**发布,不再是每晚:
规则本身是每个交易日轮动的,所以这条记录大约每五个交易日才抽样一次,
因而低估了规则自身会产生的换手——以及成本。文件是 append-only 的。整条链路——取数、选股、打分、重绘——都在
...
| 每笔交易的平均毛收益 | -0.25% |
| 上涨的交易笔数 | 6/9 |
| 事前公布的下一个持仓 | `SZ_000856` |
| 结论 | `insufficient_forward_sample` |

每一行是一笔 holding_days=1 的交易。runner 每周发布,而规则本身每个交易日轮动,
所以这些交易是抽样的、并不连续,把它们连乘并不构成一条连续的净值曲线。
不足 60 笔已打分交易,结论不会改变,无论数字怎么走。
这一段由每周任务重写,不是手写的。
<!-- LIVE-RECORD:END -->
...
[数据已公开](data/source_reconciliation.json)。
...
GitHub runner 上,十票记录跑在作者本机。单股 runner 自 2026-08-22 起为每周追加。
