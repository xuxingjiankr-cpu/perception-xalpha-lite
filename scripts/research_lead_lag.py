...
def load_day(day_dir: Path) -> dict[str, dict[str, Any]]:
    """code -> {name, full_amount, series:[(minute, price)] sorted}."""
    ...
    for gz in sorted(day_dir.glob("*_etf.csv.gz")):
        ...
        for r in rows:
            ...
            node = by_code.setdefault(code, {"name": r.get("name"), "full_amount": 0.0, "series": []})
...
def analyze_day(day: str, by_code: dict[str, dict[str, Any]], *, keyword_map: dict[str, Any],
    ...
    # eligible names + sector grouping
    sectors: dict[str, list[str]] = {}
    for code, node in by_code.items():
        if node["full_amount"] < min_amount or len(node["series"]) < 5:
            continue
    ...
    for sector, codes in sectors.items():
        if len(codes) < min_members:
            continue
        leader = max(codes, key=lambda c: by_code[c]["full_amount"])
        followers = [c for c in codes if c != leader]
