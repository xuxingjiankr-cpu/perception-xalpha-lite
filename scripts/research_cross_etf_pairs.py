...
ETF price history we already collect (legacy and daily minute quote logs). No external feed.
...
Run: py -3.13 scripts/research_cross_etf_pairs.py [--quotes FILE_OR_DIRECTORY]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Research cross-ETF relative value from local quote logs")
    parser.add_argument("--quotes", default=str(QUOTES), help="minute quote JSONL file or directory")
    args = parser.parse_args()
    sys.stdout.reconfigure(encoding="utf-8")
    rows = load_aligned(Path(args.quotes))
